...
test("AC current sources preserve DC offsets in their simulated waveforms", async () => {
  const { SpiceNetlist } = await import("lib/spice-classes/SpiceNetlist")
  const { processSimulationCurrentSources } = await import(
    "lib/processors/process-simulation-current-sources"
  )
